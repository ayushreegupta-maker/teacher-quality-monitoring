"""
Two related responsibilities, both about turning identifiers into session
context the pipeline can lean on:

1. `resolve_session_context(video_path, ...)` — given a raw video file, parse
   its filename for camera + recorded_at, look up the activity row in
   `data/tqm.db`, return a context dict (classroom, activity, rubric set,
   centre). Bridges on-disk files ↔ structured DB metadata. (Legacy callers
   `score_long_video.py` / `batch_score.py` use this.)

2. `resolve_session_segments(session_id, ...)` — given a session_id like
   `2026-05-18__D28__0900`, locate the subject-bucketed raw .mp4 segments
   that span the class window (subject derived from `cctv_cameras.xlsx`,
   1 camera = 1 subject). Returns SegmentEntry per matching file, sorted
   chronologically. Used by `scripts/run_rubric.py` (new architecture).

Both are pure-Python — no LLM, no ffmpeg, no Gemini calls.
"""
