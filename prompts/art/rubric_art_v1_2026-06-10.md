...
notes: |
  Externalised verbatim from the inline RUBRIC_PROMPT_TEMPLATE in
  scripts/run_art_rubric_test.py during migration step 7 of the TQM
  consolidation. No content change. The 5 placeholders use Python's
  str.format() style (single-brace `{var}`); literal braces in JSON
  examples are escaped `{{` / `}}` per .format() rules. When the new
  rubric.render_prompt() (step 8) wires this in, it'll keep using
  .format() — Jinja conversion is deferred to a future iteration.
inputs:
